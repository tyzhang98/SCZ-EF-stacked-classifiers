...
exclusive = ['PANSS_Negative', 'PANSS_Positive', 'PANSS_Affective', 'PANSS_Cognitive']
...
hold_metrics_list = []  
hold_params_list = []  
stratified_holdout_results = {} 
stratified_params_list = []
...
for model_name, model_type in models.items():
    stratified_holdout_results[model_name] = {}
    stratified_metrics_list = []  # 初始化stratified_metrics_list
    global_best_acc = 0
    for _ in tqdm(range(epoch)):
        ...
        stratified_holdout_results[model_name][_] = {}
        # 步骤1: 分层随机分割数据集
        ...
        stratify_features = {
            ...
            'Education_years': [(0, 8), (9, float('inf'))],  
            'Residence': [1, 2], 
            'Only_child': [1, 2],  
        ...
        stratified_holdout_sets = {}
        for feature, values in stratify_features.items():
            if feature in ["Age", "Education_years"]:
              
                X_holdout[feature] = pd.cut(X_holdout[feature], bins=[v[0] for v in values] + [float('inf')], labels=False, include_lowest=True)
                for i in range(len(values)):
                    stratified_holdout_sets[(feature, i)] = []
                    stratified_holdout_results[model_name][_][(feature, i)] = {'y_true': [], 'y_pred': []}  # 初始化结果字典
            else:
                for value in values:
                    stratified_holdout_sets[(feature, value)] = []
                    stratified_holdout_results[model_name][_][(feature, value)] = {'y_true': [], 'y_pred': []}  # 初始化结果字典


        for idx, row in X_holdout.iterrows():
            for feature, values in stratify_features.items():
                row_feature = row[feature]
                if pd.isnull(row_feature): 
                    continue

                for i, (idx, row) in enumerate(X_holdout.iterrows()):
                    stratified_holdout_sets[(feature, row_feature)].append((row.values, y_holdout[i]))
        ...
        # 对 stratified_holdout_sets 也进行标准化
        for key, holdout_set in stratified_holdout_sets.items():
            X_stratified_holdout = np.array([x[0] for x in holdout_set])
            y_stratified_holdout = np.array([x[1] for x in holdout_set])  # 获取y_stratified_holdout
            if X_stratified_holdout.size == 0:
                continue

            X_stratified_holdout = pd.DataFrame(X_stratified_holdout, columns=X_train.columns)
            X_stratified_holdout = pd.DataFrame(scaler.transform(X_stratified_holdout), columns=X_train.columns)
            stratified_holdout_sets[key] = list(zip(X_stratified_holdout.values, y_stratified_holdout))
        ...
        for key, holdout_set in stratified_holdout_sets.items():
            X_stratified_holdout = np.array([x[0] for x in holdout_set])
            y_stratified_holdout = np.array([x[1] for x in holdout_set])

            if X_stratified_holdout.size == 0:
                continue

            y_pred_prob_stratified = clf.predict_proba(X_stratified_holdout)[:, 1]
            stratified_metrics_list.append(np.array([y_stratified_holdout, y_pred_prob_stratified]))
            stratified_params_list.append(best_params)

            stratified_holdout_results[model_name][_][key]['y_true'] = y_stratified_holdout.tolist()  # 保存真实值
            stratified_holdout_results[model_name][_][key]['y_pred'] = y_pred_prob_stratified.tolist()  # 保存预测值
        ...
        # 将stratified_holdout_results保存到文件
        with open('metrics/stratified_holdout_results.pkl', 'wb') as f:
            pickle.dump(stratified_holdout_results, f)
