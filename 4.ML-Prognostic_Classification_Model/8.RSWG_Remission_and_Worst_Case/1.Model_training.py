...
exclusive = ['PANSS_Negative_Baseline', 'PANSS_Positive_Baseline', 'PANSS_Affective_Baseline', 'PANSS_Cognitive_Baseline']
...
def optuna_opti(trial, model_type, x, y):
    if model_type == 'SVM':
        ...
        clf_svm = SVC(**params)
        params = {'random_state': 0}
        params['n_estimators'] = trial.suggest_int('n_estimators1', 1, 200)
...
def get_best_stacking(best_params):
    params = {
        'random_state': 0,
        'probability': True,
        'C': best_params.get('C', 1.0),  # 如果 'C' 不存在,则使用默认值 1.0
        'class_weight': best_params.get('class_weight', None)  # 如果 'class_weight' 不存在,则使用默认值 None
    }
    clf_svm = SVC(**params)
    
    params = {
        'random_state': 0,
        'n_estimators': best_params.get('n_estimators1', 100),  # 如果 'n_estimators1' 不存在,则使用默认值 100
        'learning_rate': best_params.get('learning_rate', 0.1)  # 如果 'learning_rate' 不存在,则使用默认值 0.1
    }
    clf_ada = AdaBoostClassifier(**params)
    
    params = {
        'random_state': 0,
        'n_estimators': best_params.get('n_estimators2', 100),  # 如果 'n_estimators2' 不存在,则使用默认值 100
        'max_depth': best_params.get('max_depth', None),  # 如果 'max_depth' 不存在,则使用默认值 None
        'class_weight': best_params.get('class_weight', None)  # 如果 'class_weight' 不存在,则使用默认值 None
    }
    clf_rf = RandomForestClassifier(**params)
